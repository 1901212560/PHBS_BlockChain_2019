...
public class Block {
    ...
    private byte[] hash;                //该区块的hash
    private byte[] prevBlockHash;      //前一个区块的hash
    private Transaction coinbase;      //coinbase 交易
    private ArrayList<Transaction> txs;    //区块中包含的交易信息

    /** {@code address} is the address to which the coinbase transaction would go */
    public Block(byte[] prevHash, PublicKey address) {
        prevBlockHash = prevHash;
        coinbase = new Transaction(COINBASE, address);  //COINBASE=25
